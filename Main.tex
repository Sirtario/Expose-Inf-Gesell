...
Die Gewährleistung ethischer KI-Bildgenerierung beginnt mit einer sorgfältigen Berücksichtigung der Datensammlungs- und Annotationspraktiken. Es ist entscheidend, Schulungsdatensätze zu kuratieren, die vielfältig, repräsentativ und auf ethische 
Weise beschafft wurden. Dies beinhaltet die Berücksichtigung potenzieller Verzerrungen und schädlicher Inhalte, die im Datensatz vorhanden sein können. Um diese Probleme zu mindern, empfiehlt es sich, eine vielfältige Gruppe von Annotatoren 
einzubeziehen, den unterschiedlichen Perspektiven einbringen können und die Einführung von verzerrten oder diskriminierenden Labels verringern. Durch die Diversifizierung des Annotatoren-Pools wird es möglich, den Einfluss individueller Vorurteile 
zu minimieren und ausgewogenere und inklusivere Trainingsdaten zu erstellen.

Ein gutes Beispiel ist die in der Studie von (1. Gebru, T., Morgenstern, J., Vecchione, B., Vaughan, J.W., Wallach, H., Dauméé III, H. & Crawford, K. (2018). Datasheets for Datasets. arXiv Preprint arXiv:1803.09010) durchgeführte Untersuchung, die einen 
Fragenkatalog bereitstellt, der darauf abzielt, die Informationen zu ermitteln, die ein Datenblatt für einen Datensatz enthalten könnte, sowie einen Workflow, den Datensatz-Ersteller verwenden können, um diese Fragen zu beantworten. Die Fragen wurden in 
Abschnitte unterteilt, die eng mit den Hauptphasen des Lebenszyklus des Datensatzes übereinstimmen: Motivation, Zusammensetzung, Erfassungsprozess, Vorverarbeitung/Bereinigung/Annotation, Verwendung, Verteilung und Wartung.

Der Zweck der Datensatzerstellung und der Ersteller wurden als Fragen im Zusammenhang mit der Motivation aufgeworfen. Bei der Zusammensetzung ging es um den Typ des Datensatzes, die Anzahl der Fälle, ob er alle möglichen Fälle abdeckt und ob der Datensatz 
als vertraulich angesehen werden kann. Der Erfassungsprozess betrifft die Art und Weise, wie die Daten erfasst wurden: direkt von den betroffenen Personen oder über Dritte oder andere Quellen, ob eine ethische Überprüfung des Datensatzes durchgeführt 
wurde und welche Mechanismen zur Datenerfassung verwendet wurden. Dann wurden Fragen zur Vorverarbeitung/Bereinigung/Annotation der Daten gestellt (z. B. Diskretisierung oder Eimerung, Tokenisierung, Part-of-Speech-Markierung, SIFT-Feature-Extraktion, 
Entfernung von Instanzen, Verarbeitung von fehlenden Werten) und ob die "rohen" Daten gespeichert wurden. Einige Fragen betrafen die Verwendung des Datensatzes, z. B., ob der Datensatz bereits für bestimmte Aufgaben verwendet wurde oder für welche (weiteren)
 Aufgaben der Datensatz verwendet werden könnte. Bei der Verteilung wurde gefragt, wie der Datensatz verteilt wird (auf einer Website, einer API, GitHub) und ob der Datensatz an Dritte außerhalb der Organisation verteilt wird. Schließlich bezogen sich Fragen 
 zur Wartung darauf, wer den Datensatz unterstützen/hosten/warten wird, ob der Datensatz aktualisiert wird und wie der Manager des Datensatzes kontaktiert werden kann.

Neben einer vielfältigen und repräsentativen Datensammlung erfordert die ethische KI-Bildgenerierung auch die Berücksichtigung anderer Aspekte der Datenannotation. Es ist wichtig, klare Richtlinien und Protokolle für Annotatoren festzulegen, 
um Konsistenz und Genauigkeit im Beschriftungsprozess sicherzustellen. Dies hilft, subjektive Interpretationen zu reduzieren und eine gewisse Standardisierung einzuführen, die für ethische KI-Praktiken entscheidend ist.
